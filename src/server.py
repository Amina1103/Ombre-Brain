...
async def grow(content: str = "", items: Optional[list] = None) -> str:
    ...
    进阶(可选):若你(上层 AI)已经把长文拆成了 N 条最终正文,传 items=[条1, 条2, ...](字符串列表)即可**逐字入库**——跳过系统的二次拆分与改写,每条正文一字不动,只自动补元数据(领域/情感/标签/命名);合并到老桶也用原文追加、不再压缩。你有完整对话上下文,拆分和表述质量比只看二手长文的内部模型更高,能避免反复压缩带来的失真。items 正文必须用具名第三人称:自己写「Cyrus」,她写「Amina」,不用「我/她」等代词——原文逐字入库,日后脱离当下上下文回读时代词会失去归属。传了 items 就忽略 content;不传则按上面的默认行为整段整理。"""
    return await _with_notice(
